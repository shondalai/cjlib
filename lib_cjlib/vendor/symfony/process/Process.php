...
class Process implements \IteratorAggregate
{
	...
	public function start( ?callable $callback = null, array $env = [] ): void
    {
	    ...
	    try
	    {
		    ...
		    {
			    $process = @proc_open( 'exec ' . $this->buildShellCommandline( $commandline ), $descriptors, $this->processPipes->pipes, $this->cwd,
	...
	public function setOptions( array $options ): void
    {
        ...
        foreach ($options as $key => $value) {
            if (!\in_array($key, $existingOptions)) {
	            ...
	            throw new LogicException( \sprintf( 'Invalid option "%s" passed to "%s()". Supported options are "%s".', $key, __METHOD__,
		            implode( '", "', $existingOptions ) ) );
            }
