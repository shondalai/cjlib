...
final class Path
{
    ...
    public static function makeRelative(string $path, string $basePath): string
    {
        ...
        if ('' !== $root && '' === $baseRoot) {
	        throw new InvalidArgumentException( \sprintf( 'The absolute path "%s" cannot be made relative to the relative path "%s". You should provide an absolute base path instead.',
		        $path, $basePath ) );
        }

        // Fail if the roots of the two paths are different
        if ($baseRoot && $root !== $baseRoot) {
	        throw new InvalidArgumentException( \sprintf( 'The path "%s" cannot be made relative to "%s", because they have different roots ("%s" and "%s").',
