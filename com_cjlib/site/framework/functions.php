...
class CJFunctions {
	...
	public static function load_jquery( $params = [] ) {
		...
		foreach ( $params['libs'] as $plugin )
		{
			...
			switch ( $plugin )
			{
				...
				default:
					CjScript::_( $plugin, [ 'custom' => $custom_tag ] );
					break;
			}
	...
	public static function load_editor( $editor, $id, $name, $html, $rows, $cols, $width = null, $height = null, $class = null, $style = null, $custom_tag = false ) {
		...
		if ( $editor == 'bbcode' )
		{
			...
			$content
				     = '<style type="text/css"><!-- .markItUpHeader ul { margin: 0; } .markItUpHeader ul li	{ list-style:none; float:left; position:relative; background: none;	line-height: 100%; margin: 0; padding: 0; } --></style>';
	 ...
	 */
	public static function get_user_avatar( $system, $userid, $displayname = 'username', $height = 48, $email = null, $attribs = [], $img_attribs = [] ) {
		$api = new CjLibApi();
	 ...
	 */
	public static function get_user_avatar_image( $system, $userid, $alt, $height = 48, $email = null, $path_only = true, $attribs = [] ) {
		$api = new CjLibApi();
	 ...
	 * @deprecated
	 */
	public static function get_user_profile_link( $system, $userid = 0, $username = 'Guest', $links = [], $alias = null, $path_only = false, $attribs = [] ) {
	 ...
	 */
	public static function get_user_profile_url( $system, $userid = 0, $username = 'Guest', $path_only = true, $attribs = [] ) {
		$api = new CjLibApi();
	...
	/**
	 * Loads the comments from the installed/selected comment system. The comment system <code>type</code> should tell which comment system need to be used to load the comments
	 * from. The possible values are:<br><br> jcomment - JComments (id and title are required) <br> fbcomment - Facebook comment system (url is required)<br> disqus - Disqus
	 * comment system (id, title, identifier and url are required)<br> intensedebate - Intense Debate comment system (id, title, identifier and url are required)<br> jacomment -
	 * JAComment system (id and title are required)<br> jomcomment - JomComment (id is required)<br><br> Passing any other value will silently skips the code. In all the above
	 * cases, <code>type</code> and <code>app_name</code> are required parameters. While <code>type</code> specifies the comment system to be used, <code>app_name</code> is the
	 * Joomla extension name (ex: com_appname) which is loading the comments for its content.
	 *
...
// 						var disqus_developer = 1;
			...
			case 'jacomment':
				if ( ! $app->input->getInt( 'print' ) && file_exists( JPATH_SITE . '/components/com_jacomment/jacomment.php' )
				     && file_exists( JPATH_SITE . '/plugins/system/jacomment.php' ) )
				{
	...
	/**
	 * Downloads the file from remote server to specified location on local server. Uses regular file operations or cURL or sockets (whichever available first) to download the
	 * file.
	 *
	...
	public static function download_file( $source_url, $target_folder, $target_file ) {
		...
		elseif ( function_exists( 'fsockopen' ) && $data == '' )
		{
			$errno  = 0;
			$errstr = '';
	...
	public static function send_messages_from_queue( $records = 60, $delay = 0, $simulated = true, $force_ids = [] ) {
	...
	public static function get_joomla_categories_table_markup( $categories, $options = [] ) {
		...
		$attribs        = isset( $options['link_attribs'] ) ? $options['link_attribs'] : [];
		...
		foreach ( $categories as $category )
		{
			...
			if ( ( $itemid % $num_rows == $num_rows - 1 ) || ( $itemid + 1 == count( $categories ) ) )
			{
				$table = $table . '</div>';
			}
