...
<div class="row" id="cj-wrapper">
    ...
    <div class="<?php echo $span;?>">
    	<div id="j-main-container">
        	<form action="<?php echo Route::_('index.php?option=com_cjlib&view=countries');?>" method="post" name="adminForm" id="adminForm">
        	
        		<div class="clearfix form-inline margin-bottom-10">
        			<div class="pull-right">
        				
        				<span class="badge badge-important tooltip-hover" title="<?php echo Text::_('COM_CJLIB_ADD_LANGUAGE_HELP');?>">?</span>
        				
        				<select id="filter_language" name="filter[language]" size="1" onchange="document.adminForm.submit();">
        					<option value=""><?php echo Text::_('COM_CJLIB_FILTER_LANGUAGE');?></option>
        				...
        				<button type="button" class="btn btn-danger" onclick="document.adminForm.task.value='countries.add';document.adminForm.submit();">
        					<?php echo Text::_('COM_CJLIB_ADD_LANGUAGE');?>
        				</button>
        				<?php endif;?>
        			...
        			<select id="filter_published" name="filter[published]" onchange="this.form.submit();" class="inputbox">
        				<option value=""<?php echo $this->state->get('filter.published') ? '' : ' selected="selected"';?>><?php echo Text::_('JOPTION_SELECT_PUBLISHED')?></option>
        				<option value="-2"<?php echo $this->state->get('filter.published') == '-2' ? ' selected="selected"' : '';?>><?php echo Text::_('JTRASHED')?></option>
        				<option value="0"<?php echo $this->state->get('filter.published') == '0' ? ' selected="selected"' : '';?>><?php echo Text::_('JUNPUBLISHED')?></option>
        				<option value="1"<?php echo $this->state->get('filter.published') == '1' ? ' selected="selected"' : '';?>><?php echo Text::_('JPUBLISHED')?></option>
        				<option value="2"<?php echo $this->state->get('filter.published') == '2' ? ' selected="selected"' : '';?>><?php echo Text::_('JTRASHED')?></option>
        				<option value="*"<?php echo $this->state->get('filter.published') == '*' ? ' selected="selected"' : '';?>><?php echo Text::_('JALL')?></option>
        			</select>
        			<input type="text" name="filter_search" id="filter_search"
        				value="<?php echo $this->state->get('filter.search');?>" placeholder="<?php echo Text::_('COM_CJLIB_SEARCH')?>"/>
        			<input type="submit" value="<?php echo Text::_('COM_CJLIB_SEARCH');?>" class="btn btn-primary">
        			<input type="button" value="<?php echo Text::_('COM_CJLIB_RESET');?>" class="btn"
        		...
        		<div style="display: none;">
        			...
        			<span id="url-save-country-name"><?php echo Route::_('index.php?option=com_cjlib&task=country.save');?></span>
